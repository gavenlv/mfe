 ...
 *  - 如果入口文件直接静态 import React 并渲染，构建工具会在解析入口时
 *    把 React 当作本微前端的私有依赖打包，导致“共享依赖”失效，出现两份 React。
 *  - 解决办法：入口文件只写一个动态 import('./bootstrap.jsx')，
 *    这会延迟 bootstrap 的执行，等异步 chunk 加载时，共享依赖已准备好，
 *    bootstrap 里再 import React 才能正确复用宿主的那一份。
 *  - 这就是“异步边界（async boundary）”：用动态 import 把共享依赖的解析
 *    推迟到运行时，让 shared 生效。
 *
